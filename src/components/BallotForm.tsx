...
export const BallotForm: React.FC<BallotFormProps> = ({
  ...
  const { positions, candidates, config } = electionState;
  ...
  const [showManifestosDirectory, setShowManifestosDirectory] = useState(false);
  const [voterSidebarOpen, setVoterSidebarOpen] = useState(false);
  ...
  const progressPercent = totalPositions > 0 ? Math.round((completedPositionsCount / totalPositions) * 100) : 0;
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 space-y-6 text-slate-800">
      ...
      <div className="lg:hidden bg-white border border-slate-200 rounded-xl p-3.5 shadow-sm flex items-center justify-between gap-3 sticky top-16 z-30">
        <button
          onClick={() => setVoterSidebarOpen(true)}
        ...
        >
          ...
          <span>Voter Menu & Progress</span>
        </button>

        <div className="flex items-center gap-2">
          <span className="text-[11px] font-bold text-slate-500 font-mono">
            {completedPositionsCount}/{totalPositions} Selected
          </span>
          <span className="bg-emerald-100 text-emerald-800 font-mono text-[10px] font-extrabold px-2 py-0.5 rounded-full">
            {progressPercent}%
          </span>
        </div>
      </div>

      {/* Main Layout Grid with Left Side Menu and Right Ballot Content */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* DESKTOP SIDEBAR / VOTER MENU */}
        <aside className="hidden lg:block lg:col-span-4 sticky top-20 space-y-4">
          {/* Official Voter Session Card */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
              <div className="w-10 h-10 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-center text-[#00923f] shrink-0 shadow-2xs">
                <ShieldCheck className="w-6 h-6 text-[#00923f]" />
              </div>
              <div className="min-w-0">
                <span className="text-[10px] font-extrabold text-[#00923f] uppercase tracking-wider block">
                  UHAS-NTD CLUB
                </span>
            ...
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-slate-500 font-medium">Authorized Voter:</span>
                <span className="font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full text-[10px] font-mono">
                  Verified
                </span>
              </div>
            ...
            {config.electionTimerActive && config.electionEndTime && (
              <div className="space-y-1 pt-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                  Live Poll Closing Timer
                </span>
                <ElectionCountdown config={config} className="w-full justify-center" />
              </div>
            )}

            {/* Overall Ballot Progress */}
            <div className="space-y-2 pt-2 border-t border-slate-100">
              <div className="flex items-center justify-between text-xs">
                <span className="font-bold text-slate-700 uppercase tracking-wider text-[11px]">
                  Ballot Progress
                </span>
                <span className="font-mono font-extrabold text-[#00923f]">
                  {completedPositionsCount} of {totalPositions} Selected ({progressPercent}%)
                </span>
              </div>
              <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-emerald-500 to-[#00923f] transition-all duration-300 rounded-full"
                  style={{ width: `${progressPercent}%` }}
                />
              </div>
            </div>

            {/* Fast-Jump Positions Index */}
            <div className="space-y-1.5 pt-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">
                Ballot Positions Index
              </span>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <span className="truncate block">{pos.title}</span>
                          {selectedCand && (
                            <span
                      ...
                      {isDone ? (
                        ...
                        <span className="text-[10px] text-slate-400 shrink-0 font-mono">Pending</span>
                      )}
                    </button>
                  );
                })}
            ...
            <div className="space-y-2 pt-3 border-t border-slate-100">
              <button
                onClick={() => setShowManifestosDirectory(true)}
                className="w-full py-2.5 px-3 bg-slate-50 hover:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer"
              >
                <BookOpen className="w-4 h-4 text-[#00923f]" />
                <span>Read Nominees' Manifestos</span>
              </button>

              <button
                onClick={() => setShowRulesModal(true)}
                className="w-full py-2 px-3 text-slate-600 hover:text-slate-900 text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer"
              >
                <HelpCircle className="w-3.5 h-3.5 text-slate-400" />
                <span>Voting Rules & Anonymity</span>
              </button>

              <button
                onClick={onCancel}
                className="w-full py-2 px-3 text-rose-600 hover:text-rose-700 text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer"
              >
                <LogOut className="w-3.5 h-3.5" />
                <span>Exit Ballot Session</span>
              </button>
            </div>
          </div>
        </aside>

        {/* MAIN BALLOT CONTENT AREA (RIGHT COLUMN) */}
        <main className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-amber-600 font-extrabold uppercase tracking-wider">
                    UHAS-NTD CLUB
                  </span>
                </div>
                <h1 className="text-xl sm:text-2xl font-black text-slate-900 mt-1 tracking-tight uppercase">
                  ELECTORAL COMMISSION
                </h1>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2">
                  ...
                  <div className="text-sm font-mono font-bold text-[#00923f]">
                    {voterId}
                  </div>
                </div>
                ...
                <button
                  onClick={() => setShowManifestosDirectory(true)}
                  className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition cursor-pointer"
                >
                  <BookOpen className="w-4 h-4 text-[#00923f]" />
            ...
            {/* Step Pill Badges Navigation */}
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between text-xs text-slate-600 font-medium">
                <span className="font-bold text-slate-900 uppercase tracking-wider">
                  Position {currentStep + 1} of {totalPositions}:{' '}
                  <span className="text-[#00923f]">{activePosition?.title}</span>
                </span>
                <span className="font-mono font-bold text-slate-700">
                  {completedPositionsCount}/{totalPositions} Completed
                </span>
              </div>
          ...
          {activePosition && (
            <section className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm transition-all duration-200 space-y-6">
                    ...
                    >
                      <div className="space-y-4">
                        <div className="flex items-start justify-between gap-3 sm:gap-4">
                            ...
                            <div className="relative shrink-0">
                              {candidate.photoUrl ? (
                                <img
                                  ...
                                  className={`w-16 h-16 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-2xl object-cover border-2 shadow-xs transition shrink-0 ${
                              ...
                              <div
                                className={`w-16 h-16 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 border-2 border-slate-300 flex flex-col items-center justify-center text-slate-700 font-extrabold text-xl sm:text-3xl shrink-0 shadow-2xs ${
                                  candidate.photoUrl ? 'hidden' : ''
                                }`}
                              >
                                <span>{candidate.name ? candidate.name.charAt(0).toUpperCase() : <User className="w-6 h-6" />}</span>
                                <span className="text-[8px] sm:text-[9px] font-mono text-slate-500 font-normal uppercase tracking-wider mt-0.5">Nominee</span>
                              </div>
                            </div>

                            <div className="min-w-0 flex-1">
                              <h3 className="font-extrabold text-slate-900 text-sm sm:text-base md:text-lg group-hover:text-[#00923f] transition truncate sm:whitespace-normal">
                                {candidate.name}
                              </h3>
                              <p className="text-xs text-[#00923f] font-bold mt-0.5 truncate sm:whitespace-normal">
                                {candidate.titleAndDept}
                              </p>
                              {candidate.school && (
                                <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 mt-1">
                                  <span className="bg-slate-100 text-slate-700 text-[9px] sm:text-[10px] font-semibold px-1.5 sm:px-2 py-0.5 rounded border border-slate-200 truncate max-w-[140px] sm:max-w-none">
                                    {candidate.school}
                                  </span>
                                  {candidate.level && (
                                    <span className="bg-amber-100 text-amber-800 text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border border-amber-200 shrink-0">
                                      {candidate.level}
                                    </span>
                                  )}
                                </div>
                          ...
                          {/* Radio Button */}
                          <div
                            className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors mt-1 ${
                              isSelected
                                ? 'bg-[#00923f] border-[#00923f] text-white shadow-sm'
                                : 'border-slate-300 group-hover:border-slate-400 bg-white'
                            }`}
                          >
                            {isSelected && <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 stroke-[3]" />}
                          </div>
                        </div>
      ...
      {/* MOBILE VOTER SIDEBAR DRAWER */}
      {voterSidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden bg-slate-900/60 backdrop-blur-sm flex justify-start">
          <div className="bg-white w-5/6 max-w-sm h-full p-5 shadow-2xl flex flex-col justify-between overflow-y-auto space-y-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-[#00923f]" />
                  <div>
                    <span className="text-[10px] font-extrabold text-[#00923f] uppercase block">
                      UHAS-NTD CLUB
                    </span>
                ...
                <button
                  onClick={() => setVoterSidebarOpen(false)}
              ...
              {/* Voter Badge in Drawer */}
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-slate-500">Authorized Voter:</span>
                  <span className="font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full text-[10px] font-mono">
                    Verified
                  </span>
                </div>
              ...
              {config.electionTimerActive && config.electionEndTime && (
                <div className="space-y-1">
                  ...
                  <ElectionCountdown config={config} className="w-full justify-center text-xs" />
                </div>
              )}

              {/* Positions List in Drawer */}
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          setValidationError(null);
                          setCurrentStep(idx);
                          setVoterSidebarOpen(false);
                          window.scrollTo({ top: 0, behavior: 'smooth' });
                        }}
                        className={`w-full text-left p-2.5 rounded-xl text-xs font-bold transition flex items-center justify-between gap-2 ${
                          isActive
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="w-5 h-5 rounded-full bg-slate-200 text-slate-800 text-[10px] flex items-center justify-center shrink-0">
                            {idx + 1}
                          </span>
                          <div className="min-w-0">
                            <span className="truncate block">{pos.title}</span>
                            {selectedCand && (
                              <span className="text-[10px] font-normal text-emerald-600 truncate block">
                                ✓ {selectedCand.name}
                              </span>
                            )}
                          </div>
            ...
            <div className="space-y-2 pt-3 border-t border-slate-200">
              <button
                onClick={() => {
                  setVoterSidebarOpen(false);
                  setShowManifestosDirectory(true);
                }}
                className="w-full py-2.5 px-3 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer"
              >
                <BookOpen className="w-4 h-4 text-[#00923f]" />
                <span>Read Nominees' Manifestos</span>
              </button>
      ...
      {/* Nominees Manifesto Directory Modal */}
      {showManifestosDirectory && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto text-slate-900">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div className="flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-[#00923f]" />
                <h3 className="text-base font-extrabold uppercase tracking-tight text-slate-900">
                  Nominee Manifestos Directory
                </h3>
              </div>
              <button
                onClick={() => setShowManifestosDirectory(false)}
            ...
            <p className="text-xs text-slate-500 font-medium">
              Explore the visions, policy initiatives, and campaign promises of all candidates running in this election.
            </p>
            ...
            <div className="space-y-4 pt-1">
              {positions.map((pos) => {
                ...
                return (
                  <div key={pos.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
                    <h4 className="text-xs font-extrabold text-[#00923f] uppercase tracking-wider">
                      Position: {pos.title}
                    </h4>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            {cand.photoUrl ? (
                              ...
                              <div className="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-slate-500">
                                {cand.name.charAt(0)}
                              </div>
                            )}
                            ...
                            <div>
                              ...
                              <p className="text-xs font-bold text-slate-500">{cand.titleAndDept}</p>
                            </div>
                          </div>

                          {cand.visionStatement && (
                            <p className="text-xs italic text-slate-600 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                              "{cand.visionStatement}"
                            </p>
                          )}

                          {cand.manifesto ? (
                            <div className="text-xs text-slate-700 whitespace-pre-wrap leading-relaxed max-h-36 overflow-y-auto p-2.5 bg-emerald-50/40 rounded-lg border border-emerald-100">
                              {cand.manifesto}
                            </div>
                          ) : (
                            <p className="text-[11px] text-slate-400 italic">No extended manifesto attached.</p>
                          )}
                        </div>
                      ))}
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4 text-slate-900">
            ...
            <div className="space-y-3 text-xs text-slate-600 leading-relaxed font-medium">
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl space-y-1">
                <strong className="text-emerald-900 block font-extrabold">100% Cryptographic Anonymity</strong>
                <p>
                  Your Voter ID authorizes entry to the ballot session. Once you cast your ballot, the vote is detached and recorded anonymously. No administrator or candidate can link your choices back to your identity.
                </p>
              </div>

              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <strong className="text-slate-900 block font-bold">One Vote Per Voter ID</strong>
                <p>
                  Each Voter ID can only be used once. Once submitted, your voter status is permanently marked as VOTED.
                </p>
              </div>

              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <strong className="text-slate-900 block font-bold">Poll Closing Enforcement</strong>
                <p>
                  Votes must be submitted before the official election timer concludes. Any vote cast after the deadline will be blocked by the electoral system.
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setShowRulesModal(false)}
                className="px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer"
              >
                Got It
              </button>
            </div>
